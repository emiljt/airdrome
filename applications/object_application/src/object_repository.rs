...
async fn save_object(db: &mut sqlx::pool::PoolConnection<sqlx::MySqlConnection>, object: Object)
    -> Result<(), &'static str> {
    ...
    for target in object.targets.value.iter() {
        object_targets_values.push(format!("(@new_object_id, (SELECT `id` FROM `object_application_targets` WHERE `name` = '{}'))", target));
    }
...
pub async fn read_object(db: &mut sqlx::pool::PoolConnection<sqlx::MySqlConnection>, id: &str)
    -> Result<Object, &'static str> {
    let mut rows = sqlx::query("SELECT object.guid, object.name, object.description,
...
async fn read_objects(db: &mut sqlx::pool::PoolConnection<sqlx::MySqlConnection>, name: Option<&str>,
    targets: Option<Vec<&str>>, languages: Option<Vec<&str>>, keywords: Option<&str>)
    -> Vec<Object> {
